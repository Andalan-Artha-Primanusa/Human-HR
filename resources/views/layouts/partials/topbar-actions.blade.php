...
@php
  ...
  // ====== Upcoming Interviews (punya user) ======
  $interviewUpcoming = 0;
  ...
  if ($u && Schema::hasTable('interviews') && Schema::hasTable('job_applications')) {
      try {
          ...
          $interviewItems = (clone $base)
              ->where('interviews.start_at','>=', now()->subDays(7)) // kasih konteks seminggu ke belakang
              ->select([
                  'interviews.id',
                  'interviews.title',
                  'interviews.mode',
  ...
  @auth
    ...
    <div class="relative">
      <button type="button"
        ...
        @if($notifUnread > 0)
          <span class="absolute -top-0.5 -right-0.5 text-[10px] px-1.5 py-0.5 rounded-full bg-red-500 text-white">
            {{ $notifUnread }}
          </span>
        @endif
      </button>
...
<script>
  (function(){
    ...
    // optional: close on resize (layout shift)
    window.addEventListener('resize', closeAll);
    window.addEventListener('scroll', () => {
      // tutup kalau user scroll vertikal cukup jauh
      closeAll();
    }, { passive: true });
  })();
</script>
