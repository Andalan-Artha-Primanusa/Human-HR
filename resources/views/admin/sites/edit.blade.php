...
<div class="p-6 space-y-6">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-semibold text-slate-800">Edit Site</h1>
  ...
  <form action="{{ route('admin.sites.update', $site->id) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-5">
    @csrf
    @method('PUT')
    ...
    <div class="grid sm:grid-cols-2 gap-4">
      <div>
        ...
        @php $status = old('status', $site->status ?? 'active'); @endphp
      ...
      <div>
        ...
        <input type="text" name="description" value="{{ old('description', $site->description ?? '') }}"
               class="mt-1 w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
    ...
    <div class="flex items-center justify-between">
      <form action="{{ route('admin.sites.destroy', $site->id) }}" method="POST" onsubmit="return confirm('Hapus site ini?');" class="mr-auto">
        @csrf @method('DELETE')
