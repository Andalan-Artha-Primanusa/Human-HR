...
@section('content')
    <div class="mx-auto w-full max-w-[960px] px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <x-admin.page-header eyebrow="MANAGEMENT" title="Edit Company" description="Perbarui detail perusahaan.">
        <a href="{{ route('admin.companies.index') }}" class="ph-action">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
          Kembali
        </a>
        <button type="submit" form="companyEditForm" class="ph-action ph-action--brand">
      ...
      <section class="bg-white border shadow-sm rounded-2xl" style="border-color: {{ $BORD }}">
        <form id="companyEditForm" method="POST" action="{{ route('admin.companies.update', $record) }}" enctype="multipart/form-data" class="p-6 space-y-6 md:p-7 bg-white">
          @csrf
          @method('PUT')
